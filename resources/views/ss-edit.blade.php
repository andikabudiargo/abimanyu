{{-- resources/views/suggestion/edit.blade.php --}}
<!DOCTYPE html>
<html lang="id">
<head>
<meta charset="UTF-8"/>
<meta name="viewport" content="width=device-width, initial-scale=1.0"/>
<title>Edit Suggestion — ASN</title>
...
.btn-warning   { background: #b45309; color: #fff; border-color: #b45309; }
...
/* Edit mode notice */
.edit-notice { background: #fffbeb; border: 1px solid #fbbf24; border-radius: 6px; padding: 10px 14px; display: flex; align-items: flex-start; gap: 8px; font-size: 11px; color: #78350f; }
.edit-notice svg { width: 14px; height: 14px; flex-shrink: 0; margin-top: 1px; color: #d97706; }
...
/* Existing saved photo thumb */
.photo-thumb-existing { position: relative; width: 60px; height: 60px; border-radius: 4px; overflow: hidden; border: 1.5px solid #6ee7b7; flex-shrink: 0; }
.photo-thumb-existing img { width: 100%; height: 100%; object-fit: cover; }
.photo-thumb-existing .badge-saved { position: absolute; bottom: 0; left: 0; right: 0; background: rgba(22,101,52,0.78); font-size: 8px; color: #fff; text-align: center; padding: 1px 0; font-weight: 600; letter-spacing: 0.03em; }
.photo-thumb-existing .photo-thumb-rm { background: rgba(220,38,38,0.85); }
...
.err-box { background: #fef2f2; border: 1px solid #fecaca; border-radius: 4px; padding: 10px 14px; font-size: 12px; color: #7f1d1d; }
.err-box ul { margin-top: 5px; padding-left: 14px; }

.deleted-inputs-container { display: none; }
</style>
<link rel="stylesheet"
...
<div class="flex flex-col lg:flex-row gap-4 px-6 py-4">
    ...
    <div class="w-full lg:w-56 xl:w-60 flex-shrink-0 flex flex-col gap-3">
        ...
        <div class="c-card" id="selSummary" style="display:none;">
            <div class="c-card-header"><span class="c-section-label">Ringkasan</span></div>
            <div class="c-card-body" style="padding:10px 14px;">
                <table style="width:100%;font-size:11px;border-collapse:collapse;">
                    <tr id="sumSSRow" style="display:none;">
                        <td style="color:#9ca3af;padding:3px 0;width:70px;vertical-align:top;">No. SS</td>
                        <td style="color:#9ca3af;width:10px;vertical-align:top;">:</td>
                        <td id="sumSS" style="color:#111827;font-weight:600;font-family:monospace;font-size:10px;vertical-align:top;word-break:break-all;">—</td>
                    </tr>
                    <tr id="sumCatRow" style="display:none;">
                        <td style="color:#9ca3af;padding:3px 0;vertical-align:top;">Kategori</td>
                        <td style="color:#9ca3af;width:10px;vertical-align:top;">:</td>
                        <td id="sumCat" style="color:#111827;font-weight:500;vertical-align:top;word-break:break-word;">—</td>
                    </tr>
                    <tr id="sumThemeRow" style="display:none;">
                        <td style="color:#9ca3af;padding:3px 0;vertical-align:top;">Tema</td>
                        <td style="color:#9ca3af;width:10px;vertical-align:top;">:</td>
                        <td id="sumTheme" style="color:#111827;font-weight:500;vertical-align:top;word-break:break-word;">—</td>
                    </tr>
                </table>
            </div>
        </div>
        ...
        <div class="c-card">
            ...
            <div class="c-card-body" style="padding:10px 14px;">
                ...
                <p style="font-size:11px;color:#6b7280;margin-top:3px;">Batas: <strong class="text-red-600">{{ \Carbon\Carbon::parse($activePeriod->end_date)->format('d M Y') }}</strong></p>
    ...
    <div class="flex-1 min-w-0 flex flex-col gap-3">
        ...
        <div class="edit-notice">
            ...
            <span><strong>Mode Edit</strong> — Data telah diisi otomatis dari suggestion yang tersimpan. Ubah yang perlu, lalu klik <strong>Update SS</strong>.</span>
        </div>
        ...
        <div class="c-card">
            <div style="padding:12px 18px;display:flex;align-items:center;justify-content:space-between;flex-wrap:wrap;gap:8px;">
                <div>
                    <h1 style="font-size:14px;font-weight:600;color:#111827;margin:0;">Edit Suggestion System (SS)</h1>
                    <p style="font-size:11px;color:#6b7280;margin:3px 0 0;">Perbarui data improvement suggestion Anda</p>
                </div>
                <span class="status-badge">
                    <span class="status-dot amber"></span>
                    Editing &mdash; {{ $suggestion->ss_number ?? 'SS-XXXX' }}
                </span>
            </div>
        </div>
        ...
        {{--
            Route  : PUT /suggestion/{id}
            Named  : suggestion.update
            Method : PUT via @method('PUT')
        --}}
        <form method="POST"
              action="{{ route('suggestion.updateData', $suggestion->id) }}"
              enctype="multipart/form-data"
              id="ss-form">
            @csrf
            @method('PUT')

            {{-- Hidden inputs for tracking deleted existing photos --}}
            <div id="deleted-before-inputs" class="deleted-inputs-container"></div>
            <div id="deleted-after-inputs"  class="deleted-inputs-container"></div>
...
<div>
    ...
    @php
    $cats = [
        ['Safety',       'fa-shield-alt', 'Keselamatan Kerja'],
        ['Moral',        'fa-users',      'Etika & Budaya Kerja'],
        ['Quality',      'fa-award',      'Kualitas & Mutu Produk'],
        ['Productivity', 'fa-bolt',       'Efisiensi Proses Kerja'],
        ['Cost',         'fa-wallet',     'Penghematan Biaya'],
        ['Environment',  'fa-leaf',       'Lingkungan & Relasi Karyawan'],
        ['Delivery',     'fa-truck',      'Keamanan & Ketepatan Pengiriman'],
    ];

    // old() priority
    $oldCats = old('categories', $suggestion->categories ?? []);

    if (is_string($oldCats)) {
        $oldCats = json_decode($oldCats, true) ?? [];
    }
    @endphp
    ...
    @enderror
                        ...
                        <div>
                            ...
                            <input type="text" name="theme" id="f-theme"
                                class="f-input {{ $errors->has('theme')?'is-error':'' }}"
                                placeholder="Brief, descriptive title for this improvement suggestion"
                                value="{{ old('theme', $suggestion->theme) }}"
                                maxlength="200"/>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div>
                                ...
                                <input type="date" name="discovery_date" id="f-date"
                                    class="f-input {{ $errors->has('discovery_date')?'is-error':'' }}"
                                    value="{{ old('discovery_date', \Carbon\Carbon::parse($suggestion->discovery_date)->format('Y-m-d')) }}"
                                    max="{{ date('Y-m-d') }}"/>
                                @error('discovery_date')<p class="f-hint" style="color:#dc2626;">{{ $message }}</p>@enderror
                            </div>
                            <div>
                                <label class="f-label">Lokasi <sup>*</sup></label>
                                <input type="text" name="location" id="f-loc"
                                    class="f-input {{ $errors->has('location')?'is-error':'' }}"
                                    placeholder="e.g. Warehouse B, QC Line 3"
                                    value="{{ old('location', $suggestion->location) }}"/>
                        ...
                        <div>
                            ...
                            <textarea name="background" id="f-background" rows="4"
                                class="f-input {{ $errors->has('background')?'is-error':'' }}"
                                placeholder="Describe the current situation and context that led to this suggestion...">{{ old('background', $suggestion->background) }}</textarea>
                ...
                <div class="c-card">
                    ...
                    <div class="c-card-body space-y">
                        <div>
                            ...
                            <textarea name="root_cause" id="f-rootcause" rows="9"
                                class="f-input {{ $errors->has('root_cause')?'is-error':'' }}"
                                placeholder="Why 1: Reject rate increased &#10; → Because...">{{ old('root_cause', $suggestion->root_cause) }}</textarea>
                    ...
                    <div class="c-card-body space-y">
                        <div>
                            ...
                            <textarea name="improvement_activity" rows="6" class="f-input"
                                placeholder="1. [Date] Action taken — Person responsible — Outcome&#10;2. [Date] Second action...">{{ old('improvement_activity', $suggestion->improvement_activity) }}</textarea>
                        </div>
                    </div>
                    ...
                    <div style="border-top:1px solid #f3f4f6;padding:16px 18px;">
                        ...
                        <div class="g2">
                            ...
                            <div>
                                ...
                                @php $photosBefore = $suggestion->photos_before ?? []; @endphp
                                @if(count($photosBefore) > 0)
                                <div class="photo-thumbs" id="existing-thumbs-before" style="margin-bottom:8px;">
                                    @foreach($photosBefore as $i => $photo)
                                    <div class="photo-thumb-existing" id="ex-before-{{ $i }}">
                                        <img src="{{ asset('storage/' . $photo) }}" alt="Before"/>
                                        <div class="badge-saved">Tersimpan</div>
                                        <button type="button" class="photo-thumb-rm"
                                            onclick="removeExistingPhoto('before', {{ $i }}, '{{ $photo }}')">✕</button>
                                    </div>
                                    @endforeach
                                </div>
                                <p style="font-size:10px;color:#6b7280;margin-bottom:6px;">Foto tersimpan di atas. Upload baru akan <em>ditambahkan</em> (bukan menggantikan).</p>
                                @endif
                                ...
                                <div class="drop-zone" id="zone-before"
                                    ...
                                    <p style="font-size:11px;font-weight:500;color:#374151;">Click atau drag untuk upload baru</p>
                            ...
                            <div>
                                ...
                                @php $photosAfter = $suggestion->photos_after ?? []; @endphp
                                @if(count($photosAfter) > 0)
                                <div class="photo-thumbs" id="existing-thumbs-after" style="margin-bottom:8px;">
                                    @foreach($photosAfter as $i => $photo)
                                    <div class="photo-thumb-existing" id="ex-after-{{ $i }}">
                                        <img src="{{ asset('storage/' . $photo) }}" alt="After"/>
                                        <div class="badge-saved">Tersimpan</div>
                                        <button type="button" class="photo-thumb-rm"
                                            onclick="removeExistingPhoto('after', {{ $i }}, '{{ $photo }}')">✕</button>
                                    </div>
                                    @endforeach
                                </div>
                                <p style="font-size:10px;color:#6b7280;margin-bottom:6px;">Foto tersimpan di atas. Upload baru akan <em>ditambahkan</em> (bukan menggantikan).</p>
                ...
                <div class="c-card">
                    ...
                    <div class="c-card-body space-y">
                        <div class="info-banner success">
                            ...
                            <span>Melengkapi bagian ini meningkatkan skor evaluasi Anda. Data kuantitatif sebelum/sesudah diperhitungkan besar oleh manager.</span>
                        </div>
                        <div>
                            <label class="f-label">Evaluasi Hasil</label>
                            <textarea name="evaluation_result" rows="5" class="f-input"
                                placeholder="Bandingkan kondisi sebelum dan sesudah dengan data kuantitatif jika memungkinkan.&#10;&#10;Contoh: Reject rate turun dari 8.2% menjadi 2.3% dalam 3 bulan setelah implementasi.">{{ old('evaluation_result', $suggestion->evaluation_result) }}</textarea>
                        </div>
                        <div>
                            ...
                            <input type="text" name="standardization" class="f-input"
                                ...
                                value="{{ old('standardization', $suggestion->standardization) }}"/>
                            <p class="f-hint">Referensi SOP, instruksi kerja, atau nomor form yang dibuat dari improvement ini.</p>
                        </div>
                    </div>
                    <div class="c-card-footer">
                        ...
                        <div style="display:flex;gap:6px;">
                            {{-- Save and re-submit for review --}}
                            <button type="button" onclick="submitSS('submit')" id="btn-submit" class="btn btn-warning">
...
document.addEventListener('DOMContentLoaded', function () {
    // Category tile checkbox
    document.querySelectorAll('.cat-tile input').forEach(cb => {
        cb.addEventListener('change', function () {
            this.closest('.cat-tile').classList.toggle('is-checked', this.checked);
            updateSummary();
        });
    });
    ...
    if (themeInput) themeInput.addEventListener('input', updateSummary);
...
function updateSummary() {
    ...
    const ssNum = '{{ $suggestion->ss_number ?? '' }}';
    let any = false;

    // SS number (always show if present)
    if (ssNum) {
        document.getElementById('sumSS').textContent = ssNum;
        document.getElementById('sumSSRow').style.display = 'table-row';
        any = true;
    }

    if (cats.length) {
        document.getElementById('sumCat').textContent = cats.join(', ');
        document.getElementById('sumCatRow').style.display = 'table-row';
        any = true;
    } else {
        document.getElementById('sumCatRow').style.display = 'none';
    }

    if (theme) {
        document.getElementById('sumTheme').textContent = theme.length > 32 ? theme.slice(0, 32) + '…' : theme;
        document.getElementById('sumThemeRow').style.display = 'table-row';
        any = true;
    } else {
        document.getElementById('sumThemeRow').style.display = 'none';
    }
...
function validateStep(step) {
    if (step === 1) {
        if (!document.querySelectorAll('input[name="categories[]"]:checked').length) {
            alert('Pilih minimal satu kategori.'); return false;
        }
        const checks = [
            ['f-theme',      'Tema / Judul wajib diisi.'],
            ['f-date',       'Tanggal penemuan wajib diisi.'],
            ['f-loc',        'Lokasi wajib diisi.'],
            ['f-background', 'Latar belakang wajib diisi.'],
        ];
    ...
    if (step === 2) {
        ...
        if (!rc.value.trim()) {
            rc.classList.add('is-error');
            rc.focus();
            rc.addEventListener('input', () => rc.classList.remove('is-error'), { once: true });
            alert('Analisa root cause wajib diisi.');
            return false;
        }
    }
    return true;
...
/**
 * Simpan sebagai draft (update data, status tetap draft).
 * Tambahkan hidden flag save_as_draft=1 agar controller bisa membedakan.
 */
function saveDraft() {
    if (!validateStep(1) || !validateStep(2)) {
        // Re-navigate to offending step
        return;
    }
    let inp = document.getElementById('_save_as_draft');
    if (!inp) {
        inp = document.createElement('input');
        inp.type  = 'hidden';
        inp.name  = 'save_as_draft';
        inp.id    = '_save_as_draft';
        inp.value = '1';
        document.getElementById('ss-form').appendChild(inp);
    }
    document.getElementById('ss-form').submit();
}

/**
 * Submit (update + re-submit status ke 'submitted').
 * Menggunakan fetch agar response JSON bisa ditangani.
 */
async function submitSS(action = 'submit') {
    ...
    btn.disabled = true;
    btn.innerHTML = '<svg fill="none" stroke="currentColor" viewBox="0 0 24 24" style="width:12px;height:12px;"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v1m0 14v1M4.22 4.22l.71.71m12.02 12.02l.71.71M1 12h1m18 0h1M4.22 19.78l.71-.71M18.36 5.64l.71-.71"/></svg> Memperbarui...';

    try {
        const formData = new FormData(form);
        // Ensure _method=PUT is included for multipart/form-data
        formData.set('_method', 'PUT');
        formData.set('action', action);

        const response = await fetch("{{ route('suggestion.updateData', $suggestion->id) }}", {
            method: 'POST',
            headers: {
...
/**
 * Preview newly-selected photos in the thumb strip.
 */
    ...
    Array.from(input.files).forEach(file => {
        ...
        reader.onload = e => {
            ...
            div.innerHTML = `<img src="${e.target.result}"/>
                <button type="button" class="photo-thumb-rm" onclick="this.parentElement.remove()">✕</button>`;
...
/**
 * Mark an existing (saved) photo for deletion.
 * - Hides the thumb immediately.
 * - Appends a hidden input delete_photos_before[] or delete_photos_after[]
 *   so the controller knows which storage paths to remove.
 *
 * @param {string} side    'before' | 'after'
 * @param {number} index   loop index
 * @param {string} path    storage relative path (e.g. "suggestion-photos/abc.jpg")
 */
function removeExistingPhoto(side, index, path) {
    const el = document.getElementById(`ex-${side}-${index}`);
    if (el) el.style.display = 'none';

    const container = document.getElementById(`deleted-${side}-inputs`);
    const inp = document.createElement('input');
    inp.type  = 'hidden';
    inp.name  = `delete_photos_${side}[]`;
    inp.value = path;
    container.appendChild(inp);
}

// ── Auto-jump to step with validation errors ──
@if($errors->has('root_cause'))
    goStep(2);
@elseif($errors->has('categories') || $errors->has('theme') || $errors->has('discovery_date') || $errors->has('location') || $errors->has('background'))
    goStep(1);
@endif
</script>
</body>
