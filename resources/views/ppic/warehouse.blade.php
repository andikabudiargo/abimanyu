...
@section('page-title', 'DASHBOARD WAREHOUSE')
@section('breadcrumb-item', 'Logistic')
@section('breadcrumb-active', 'Warehouse')

@section('content')

   <div class="bg-white shadow rounded-xl p-6 mb-6">
    <h2 class="text-lg font-semibold mb-4">Filter Warehouse</h2>

    <form id="filter-form">
        <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
            <div>
                <label for="filterCustomer" class="block text-sm mb-1 font-medium text-gray-700">Warehouse Code</label>
            ...
            <div>
                <label for="filterCustomer" class="block text-sm mb-1 font-medium text-gray-700">Warehouse Type</label>
                <input type="text" class="w-full px-3 py-1 text-base border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500"/>
            </div>
            <div>
                <label for="filterCustomer" class="block text-sm mb-1 font-medium text-gray-700">PIC</label>
                <input type="text" class="w-full px-3 py-1 text-base border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500"/>
            </div>
        </div>
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
...
<div class="table-responsive bg-white shadow rounded-xl p-6 mb-2">
    <h2 class="text-lg font-semibold mb-2">Warehouse List</h2>
    <div class="w-full overflow-x-auto">
        <table id="warehouse-table" class="w-full text-sm text-left">
...
<div id="createModal" class="fixed inset-0 bg-black bg-opacity-30 flex items-center justify-center z-50 hidden">
  <div class="bg-white w-full max-w-lg rounded-xl shadow-lg p-6 relative">
    <h3 class="text-lg font-semibold mb-4">Create Warehouse</h3>
...
<script>
  ...
  document.getElementById('createWarehouseForm').addEventListener('submit', function(e) {
   ...
   fetch('{{ route("ppic.warehouse.store") }}', {
      method: 'POST',
      body: formData,
      headers: {
  ...
  $(document).ready(function () {
    ...
    feather.replace(); // <-- WAJIB di sini
      ...
      ajax: '{{ route("ppic.warehouse.data") }}',
