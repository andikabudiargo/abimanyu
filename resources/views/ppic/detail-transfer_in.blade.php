...
@section('title', 'Detail Transfer In')
@section('page-title', 'DETAIL TRANSFER IN')
@section('breadcrumb-item', 'Transfer In')
@section('breadcrumb-active', 'Detail Transfer In')
  ...
  <div class="w-full flex flex-col space-y-6">
    ...
    <div class="border border-gray-200 bg-white shadow-md rounded-xl p-6">
     <div class="flex items-center mb-4 gap-2">
      <i data-feather="info" class="text-gray-700 w-5 h-5"></i> <h3 class="text-xl font-semibold text-gray-700">Transfer In Information</h3>
  </div>
      ...
      <div class="grid grid-cols-2 gap-x-12 gap-y-6 text-sm mb-8">
        ...
        <div>
          ...
          <div class="text-gray-800">{{ $transfer->transfer_category ?? '-' }}</div>
        </div>
        @if($transfer->transfer_category === 'Incoming')
        <div>
          <div class="text-gray-500 font-medium mb-1">Supplier</div>
          <div class="text-gray-800">{{ optional($transfer->supplier)->name ?? '-' }}</div>
        </div>
        @elseif($transfer->transfer_category === 'Material Return')
        <div>
          <div class="text-gray-500 font-medium mb-1">From</div>
          <div class="text-gray-800">{{ optional($transfer->fromLocation)->name ?? '-' }}</div>
        </div>
        @endif
         
      </div>
  <div class="flex items-center mb-4 mt-8 gap-2">
      <i data-feather="package" class="text-gray-700 w-5 h-5"></i> <h3 class="text-xl font-semibold text-gray-700">Transfer In Items</h3>
  </div>
    ...
    <tr>
        ...
        <td class="p-3 border text-center">{{ $item->uom ?? ($item->article->unit ?? '-') }}</td>
        <td class="p-3 border text-center">{{ $item->min_package ?? ($item->article->min_package ?? 1) }}</td>
