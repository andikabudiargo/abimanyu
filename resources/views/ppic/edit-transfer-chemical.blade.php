...
@section('title', 'Edit Transfer Chemical')
@section('page-title', 'EDIT TRANSFER CHEMICAL')
@section('breadcrumb-item', 'PPIC')
@section('breadcrumb-active', 'Edit Transfer Chemical')
...
<div class="space-y-4">
  ...
  <div class="w-full bg-white shadow-md rounded-xl px-8 space-y-4 pt-6 pb-12">
    <div class="flex items-start gap-3 border-b border-gray-200 pb-3 mb-6 -mx-8 px-8">
      <div>
        <h2 class="text-base font-semibold text-slate-800 tracking-wide">Edit Transfer Chemical</h2>
        <p class="text-sm text-gray-500">Edit and update chemical stock transfer with automated unit conversion</p>
      </div>
    </div>
      ...
      <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <div>
          ...
          <select name="location_from" id="location_from"
            ...
            @foreach(['Warehouse Chemical','Spraybooth 1A','Spraybooth 1B','Spraybooth 1C','Spraybooth 2A','Spraybooth 2B','Spraybooth 2C','Spraybooth 3A','Spraybooth 3B','Spraybooth 3C','Spraybooth 4A','Spraybooth 4B','Spraybooth 4C','Spraybooth 5A','Spraybooth 5B','Spraybooth 5C'] as $loc)
              <option value="{{ $loc }}" {{ $transfer->location_from === $loc ? 'selected' : '' }}>{{ $loc }}</option>
            @endforeach
          </select>
        </div>
        ...
        <div>
          ...
          <select name="location_to" id="location_to"
            ...
            <option value="">-- Pilih Lokasi --</option>
            @foreach(['Warehouse Chemical','Spraybooth 1A','Spraybooth 1B','Spraybooth 1C','Spraybooth 2A','Spraybooth 2B','Spraybooth 2C','Spraybooth 3A','Spraybooth 3B','Spraybooth 3C','Spraybooth 4A','Spraybooth 4B','Spraybooth 4C','Spraybooth 5A','Spraybooth 5B','Spraybooth 5C'] as $loc)
              <option value="{{ $loc }}" {{ $transfer->location_to === $loc ? 'selected' : '' }}>{{ $loc }}</option>
            @endforeach
          </select>
        </div>
      ...
      <div>
        ...
        <textarea name="note" id="note" rows="3"
          ...
          class="w-full px-3 py-2 bg-gray-50 border border-gray-300 rounded-lg shadow-sm text-sm
                 focus:outline-none focus:ring-2 focus:ring-blue-400 focus:border-blue-400 transition">{{ $transfer->note }}</textarea>
      </div>
  ...
  <div class="w-full bg-white shadow-md rounded-xl p-8 space-y-4">
    <div class="flex items-center gap-2 border-b border-gray-200 pb-2 mb-4">
      <p class="text-sm font-semibold text-slate-800">Edit List Chemical</p>
    </div>
...
<script>
  // =========================================================
  // DATA EXISTING dari controller (inject via Blade)
  // =========================================================
 const existingItems = @json($existingItems);
  ...
  $(document).ready(function () {
    ...
    // Trigger change agar summary langsung tampil
    $('#location_from, #location_to').trigger('change');
  });
  ...
  function syncLocationOptions() {
    ...
    if (from) $('#location_to option[value="' + from + '"]').prop('disabled', true);
    if (to)   $('#location_from option[value="' + to + '"]').prop('disabled', true);
  ...
  $(function () {
    // Load existing items, fallback ke satu baris kosong
    if (existingItems && existingItems.length > 0) {
      chemicals = existingItems;
    } else {
  ...
  function initSelect2() {
    $('.chemicalSelect').each(function () {
      ...
      // Restore saved selection (existing data)
      if (row.id && row.text) {
      ...
      $el.on('select2:select', function (e) {
        ...
        chemicals[index].article_code = item.article_code;
  ...
  $('#transferForm').on('submit', function (e) {
    ...
    $btn.prop('disabled', true)
        .html('<svg class="animate-spin h-4 w-4 mr-1 inline" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"><circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle><path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8v8z"></path></svg> Updating...');
    ...
    $.ajax({
      ...
      success: function () {
        Swal.fire({
          ...
          position:          'top-end',
          icon:              'success',
          title:             'Transfer berhasil diupdate!',
          showConfirmButton: false,
          timer:             3000,
          timerProgressBar:  true
        }).then(() => {
          window.location.href = '{{ route("ppic.tfcm1.index") }}';
        });
      },
      error: function (xhr) {
      ...
      complete: function () {
        $btn.prop('disabled', false)
            .html('<i data-feather="save" class="h-4 w-4"></i> Update');
        feather.replace();
      }
    });
  });
