...
@section('title', 'Edit ATK Request')
@section('page-title', 'EDIT ATK REQUEST')
@section('breadcrumb-item', 'GA Inventory Management')
@section('breadcrumb-active', 'Edit ATK Request')
...
<div class="bg-white shadow rounded-xl overflow-hidden">
  ...
  <div class="flex items-center justify-between px-6 py-4 border-b border-gray-100">
    <div class="flex items-center gap-3">
      ...
      <div>
        <h2 class="text-sm font-semibold text-gray-800">Edit ATK Request</h2>
  ...
  <div class="px-6 py-5 space-y-6">
        ...
        <div>
          ...
          <div class="flex items-center gap-2 py-2 rounded-lg">
            ...
            <span class="text-xs font-medium text-gray-700">
              {{ \Carbon\Carbon::parse($atkRequest->created_at)->translatedFormat('d F Y') }}
            </span>
          </div>
        </div>
    ...
    <div>
      ...
      <textarea id="requestNotes" rows="3"
        ...
        class="w-full border border-gray-200 rounded-lg px-3 py-2.5 text-xs focus:outline-none focus:ring-2 focus:ring-blue-400 resize-none placeholder-gray-300 text-gray-700">{{ $atkRequest->note }}</textarea>
    </div>
  ...
  <div class="flex items-center justify-between px-6 py-4 border-t border-gray-100 bg-gray-50/50">
    ...
    <div class="flex items-center gap-2">
      <button type="button" onclick="submitEdit()" id="btnSubmitEdit"
        class="flex items-center gap-1.5 px-5 py-2 bg-amber-500 hover:bg-amber-600 text-white text-xs font-medium rounded-lg shadow transition">
...
{{-- Row Template (sama dengan create) --}}
...
// Existing items dari backend
const existingItems = @json($atkRequest->items->map(fn($i) => [
    'atk_id' => $i->atk_id,
    'qty'    => $i->qty,
]));
...
$.ajax({
    ...
    success: function (res) {
        ...
        if (existingItems.length > 0) {
            existingItems.forEach(item => addRequestRow(item.atk_id, item.qty));
        } else {
            addRequestRow();
        }
    }
...
function addRequestRow(preselectedAtkId = null, preselectedQty = null) {
    ...
    // Preselect jika ada data existing
    if (preselectedAtkId) {
        $sel.val(preselectedAtkId).trigger('change.select2');
        onAtkChange($row, preselectedQty);
    }
...
function onAtkChange($row, presetQty = null) {
    ...
    $row.find('.req-qty').attr('max', balance);

    if (presetQty !== null) {
        $row.find('.req-qty').val(presetQty);
    }
...
function validateQty($row) {
    ...
    if (balance <= 0) {
        ...
        showWarning(`Stok "${atk.name}" kosong.`);
        return;
    }
    $qtyInput.prop('disabled', false);
    if (qty > balance) {
        $qtyInput.val('').addClass('border-red-400 bg-red-50').removeClass('border-gray-200');
        showWarning(`Qty untuk "${atk.name}" tidak boleh melebihi stok (${balance} ${atk.uom ?? ''}).`);
        setTimeout(() => {
...
function checkDuplicates() {
    ...
    $('#requestRows [data-request-row]').each(function (i) {
        ...
        if (!val) return;
        if (seen[val]) { seen[val].rows.push(i + 1); }
        else { seen[val] = { name, rows: [i + 1] }; }
    });
    Object.values(seen).forEach(({ name, rows }) => {
        if (rows.length > 1) msgs.push(`"${name}" duplikat di baris ${rows.join(', ')}`);
    });
    msgs.length > 0 ? showWarning('Item duplikat: ' + msgs.join(' · ')) : hideWarning();
}
...
function submitEdit() {
    ...
    $('#requestRows [data-request-row]').each(function () {
        ...
        if (!atkId) { showWarning('Pilih item ATK di semua baris.'); valid = false; return false; }
        if (seen[atkId]) { showWarning('Terdapat item duplikat.'); valid = false; return false; }
        const atk     = reqAtkOptions.find(a => a.id == atkId);
    ...
    $.ajax({
        ...
        data   : JSON.stringify({ _method: 'PUT', items, notes: $('#requestNotes').val() }),
        headers: { 'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content') },
        success: function (res) {
            Swal.fire({
                icon: 'success', title: 'Berhasil!',
                text: res.message ?? 'Request berhasil diperbarui.',
        ...
        complete: function () {
            $btn.prop('disabled', false).html('<i data-feather="save" class="w-3.5 h-3.5 inline"></i> Simpan Perubahan');
            feather.replace();
        }
    });
}
