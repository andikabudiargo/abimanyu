    ...
    $('#btnAddRowMobile').off('click').on('click', function () {
        ...
        // ══════════════════════════════════════════════════════════
    // SEARCH ARTICLE (Desktop) & SEARCH PART (Mobile)
    // ══════════════════════════════════════════════════════════
    function getRowSearchText($row) {
        const code = $row.find('.article-code').val() || '';
        const name = $row.find('.part-select').find(':selected').text() || '';
        const addr = $row.find('.row-addr-label').text()
                  || $row.find('.mobile-addr-label').text()
                  || '';
        const loc  = $row.find('.location-input').val() || '';
        return (code + ' ' + name + ' ' + addr + ' ' + loc).toLowerCase();
    }

    function filterStoRows(keyword, $rows) {
        const kw = (keyword || '').trim().toLowerCase();
        $rows.each(function () {
            const $row = $(this);
            if (!kw) { $row.show(); return; }
            $row.toggle(getRowSearchText($row).includes(kw));
        });
    }

    // Desktop: filter <tr class="sto-row"> di dalam #article-table
    $(document).on('input', '#searchArticle', function () {
        filterStoRows($(this).val(), $('#article-table').find('tr.sto-row'));
    });

    // Mobile: filter <div class="sto-row"> di dalam #mobile-article-list
    $(document).on('input', '#searchPartMobile', function () {
        filterStoRows($(this).val(), $('#mobile-article-list').find('.sto-row'));
    });
