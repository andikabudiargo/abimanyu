...
.sto-mode-badge.mode-edit {
  background: rgba(245,158,11,.2); border: 1px solid rgba(245,158,11,.4); color: #FDE68A;
}
.sto-mode-badge.mode-superuser {
...
.sto-mode-badge.mode-superuser .dot { background: #69F0AE; }

/* ── TOOLBAR ─────────────────────────────────────────────────── */
.sto-toolbar {
  background: var(--sto-white); border-bottom: 1px solid var(--sto-border);
  padding: 0 32px; display: flex; align-items: center; min-height: 46px;
}
.sto-tab {
...
.sto-alert-banner.amber {
  background: var(--sto-amber-light); border: 1px solid var(--sto-amber-border); color: #92400E;
}
.sto-alert-banner.blue {
  background: var(--sto-blue-light); border: 1px solid var(--sto-blue-border); color: var(--sto-blue);
}
...
/* Qty-2 kolom highlight di thead */
.sto-table thead th.th-qty2 {
  background: rgba(0,0,0,.12);
}

.sto-table tbody tr {
  border-bottom: 1px solid var(--sto-border); transition: background .12s;
}
...
.sto-table th, .sto-table td {
  padding: 6px 8px; vertical-align: middle; overflow: hidden; white-space: nowrap;
}
...
/* ── Qty-2 cell highlight ──────────────────────────────────── */
.td-qty2 { background: rgba(232,245,233,.6); }
.td-qty2:hover { background: rgba(200,230,201,.5) !important; }
.sto-table .qty2-input {
  background: var(--sto-green-light) !important;
  border-color: #A5D6A7 !important;
}
.sto-table .qty2-input:focus {
  border-color: var(--sto-green-mid) !important;
...
.sto-row-num {
  ...
  background: var(--sto-blue-light); color: var(--sto-blue);
...
/* ── Already-saved row ─────────────────────────────────────── */
.sto-row.already-saved { opacity: .55; }
.sto-row.already-saved td { background: #F1F5F9 !important; }
.saved-tag {
  display: inline-flex; align-items: center; gap: 3px;
  ...
  <div class="sto-card">
    ...
    <div class="sto-header">
      ...
      <div style="display:flex; align-items:center; gap:12px;">
        {{-- Mode badge --}}
        @if($isSuperUser)
          <div class="sto-mode-badge mode-superuser">
    ...
    <div class="sto-body">
      ...
      @if($isSuperUser)
        <div class="sto-alert-banner blue mb-6" style="background:rgba(13,71,161,.06); border-color:var(--sto-blue-border);">
          ...
          <span>
            Mode <strong>Superuser</strong> — kedua kolom Qty dan Qty 2 dapat diinput sekaligus.
            Qty → <strong>Verificator 1</strong> &nbsp;|&nbsp; Qty 2 → <strong>Verificator 2</strong>.
          </span>
        </div>
      @elseif($isSecondUser)
        <div class="sto-alert-banner amber mb-6">
          ...
          <span>
            Anda login sebagai <strong>Verificator 2</strong>. Input Qty akan disimpan ke kolom <strong>Qty 2</strong>.
          </span>
        </div>
      @else
        <div class="sto-alert-banner blue mb-6">
          ...
          <span>
            Anda login sebagai <strong>Verificator 1</strong>. Input Qty akan disimpan ke kolom <strong>Qty</strong>.
          </span>
        </div>
      @endif
        ...
        <table class="sto-table">
          <thead>
            <tr>
              ...
              {{-- Kolom qty: conditional berdasarkan role --}}
              @if($isSuperUser)
          ...
          <tr class="sto-row {{ $item->already_saved ? 'already-saved' : '' }}" data-row="{{ $i }}">
            <input type="hidden" name="articles[{{ $i }}][other_name]"
            ...
            {{-- Qty columns: conditional --}}
            @if($isSuperUser)
              {{-- Superuser: dua kolom, tampilkan nilai existing + editable --}}
              <td class="center">
                <input type="number" min="0"
                  ...
                  placeholder="{{ $item->qty !== null ? '' : '—' }}">
              </td>
              <td class="center td-qty2">
                <input type="number" min="0"
                  name="articles[{{ $i }}][qty_2]"
                  value="{{ $item->qty_2 ?? '' }}"
                  class="qty2-input sto-input"
                  style="text-align:center;"
                  placeholder="{{ $item->qty_2 !== null ? '' : '—' }}">
              </td>
            @elseif($isSecondUser)
              {{-- Verificator 2: hanya qty_2 --}}
              <td class="center">
                <input type="number" min="0"
                  name="articles[{{ $i }}][qty_2]"
                  value="{{ $item->qty_2 ?? '' }}"
            ...
            <td class="center">
              <input type="text"
                ...
                class="part-min-package sto-input readonly" readonly
                style="text-align:center;">
            </td>
            ...
            <td class="center">
              ...
              @else
                <input type="text"
                  ...
                  value="{{ $item->location }}"
                  readonly class="location-input sto-input readonly"
                  style="text-align:center;">
              @endif
            </td>
          </tr>
