...
@section('title', 'Edit Request Perbaikan Fasilitas')
@section('page-title', 'Edit Request Perbaikan Fasilitas')
@section('breadcrumb-item', 'Facility Issue Management')
@section('breadcrumb-active', 'Edit Request Perbaikan Fasilitas')
@section('content')
    ...
    <div class="bg-white shadow-md rounded-xl p-4 space-y-4">
        ...
        <div class="flex items-start sm:items-center space-x-3">
            ...
            <div class="text-sm">
                ...
                <span class="text-gray-700">{{ $issue->creator->name ?? '-' }}</span>
            </div>
        </div>
        ...
        <div class="flex items-start sm:items-center space-x-3">
            ...
            <div class="text-sm">
                ...
                <span class="text-gray-700">{{ optional($issue->creator->departments->first())->name ?? '-' }}</span>
            </div>
        </div>
        ...
        <div class="flex items-start sm:items-center space-x-3">
            ...
            <div class="text-sm">
                ...
                <span class="text-gray-700">{{ $issue->creator->email ?? $user->creator->phone ?? '-' }}</span>
            </div>
        </div>
        ...
        <div class="flex items-start sm:items-center space-x-3">
            ...
            <div class="text-sm">
               ...
               <span class="text-gray-700">{{ $issue->created_at ?? '-' }}</span>
            </div>
        </div>
    </div>
        </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                <div>
                    ...
                    <textarea name="location_area" id="location_area" rows="3"
                              ...
                              placeholder="Masukkan lokasi area perbaikan..." required>{{ old('location_area', $issue->location_area) }}</textarea>
                </div>
                    ...
                    <div class="flex flex-wrap -mx-2">
                        ...
                        <label class="inline-flex items-center w-1/3 px-2 mb-2">
                            <input type="radio" name="request_type" value="{{ $type }}" class="form-radio text-indigo-600" required
                                {{ old('request_type', $issue->request_type) == $type ? 'checked' : '' }}>
                ...
                <div>
                    <label for="attachments" class="block text-sm font-medium text-gray-700 mb-1">
                        Unggah Foto Kerusakan <small class="text-gray-400">(Opsional)</small>
                    </label>
                ...
                <div>
                    ...
                    <select id="urgency" name="urgency" required
                        ...
                        <option value="normal" class="text-green-500" {{ old('urgency', $issue->urgency) == 'normal' ? 'selected' : '' }}>Normal / Rutin</option>
                        <option value="segera" class="text-yellow-500" {{ old('urgency', $issue->urgency) == 'segera' ? 'selected' : '' }}>Segera</option>
                        <option value="darurat" class="text-red-500" {{ old('urgency', $issue->urgency) == 'darurat' ? 'selected' : '' }}>Darurat</option>
                    </select>
                </div>
                ...
                <div class="col-span-2">
                    ...
                    <textarea name="recommendation" id="recommendation" rows="5"
                              class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm px-4 py-2 focus:ring-indigo-500 focus:border-indigo-500 resize-none">{{ old('recommendation', $issue->recommendation) }}</textarea>
                </div>
            </div>
...
$('#issue-form').off('submit').on('submit', function (e) {
    ...
    $.ajax({
        url: '{{ route("it.issue.update", $issue->id) }}',
        ...
        success: function (res) {
            if (res.success) {
                showToast('success', res.message || 'Request updated successfully!');
                setTimeout(() => {
                    window.location.href = '{{ route("it.issue.index") }}';
                }, 2000);
            } else {
                showToast('error', res.message || 'Gagal update request.');
            }
        },
        error: function (err) {
