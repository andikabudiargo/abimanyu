...
@foreach($capa->auditors as $i => $auditor)
<tr class="auditor-row">
    <td class="px-3 py-2 text-gray-400">{{ $i+1 }}</td>

    <td class="px-3 py-2">
       <select name="auditors[]"
    class="auditor-select w-full px-3 py-2 text-sm border rounded-md">
            ...
            @foreach($users as $user)
                <option value="{{ $user->id }}"
                    {{ $auditor->user_id == $user->id ? 'selected' : '' }}>
                    {{ $user->name }}
                </option>
            @endforeach
...
<section class="space-y-8">
   ...
   <div class="flex flex-wrap gap-2">
    ...
    @php
        $selectedCategory = old('category', $capa->category ?? 'Critical');
    @endphp
    ...
    <input type="radio"
        ...
        id="cat_critical"
        value="Critical"
        {{ $selectedCategory == 'Critical' ? 'checked' : '' }}
    ...
    <input type="radio"
        ...
        id="cat_major"
        value="Major"
        {{ $selectedCategory == 'Major' ? 'checked' : '' }}
    ...
    <input type="radio"
        ...
        id="cat_minor"
        value="Minor"
        {{ $selectedCategory == 'Minor' ? 'checked' : '' }}
    ...
    <input type="radio"
        ...
        id="cat_observation"
        value="Observation"
        {{ $selectedCategory == 'Observation' ? 'checked' : '' }}
...
<div>
    ...
    <div>
       ...
       <input type="text"
 name="detail_of_information"
 value="{{ old('detail_of_information', $capa->detail_of_information) }}"
 required
    ...
    <div>
        ...
        <textarea name="problem" rows="5"
            ...
            focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500">{{ old('problem', $capa->problem) }}</textarea>
    </div>
...
<div class="flex justify-between items-center pt-8 border-t">
    <a href="{{ route('mr.capa.index') }}"
        class="px-4 py-2 text-sm bg-gray-100 hover:bg-gray-200 rounded-lg text-gray-700">
        ← Back
    </a>

    <button type="submit" id="submitBtn"
        class="px-6 py-2.5 bg-green-600 hover:bg-green-700 text-white text-sm font-medium rounded-lg shadow">
        Update
    </button>
</div>
...
$('#capa-form').off('submit').on('submit', function (e) {
    ...
    $.ajax({
        url: '{{ route("mr.capa.update", $capa->id) }}',
        method: 'POST',
        data: formData,
        processData: false,
        contentType: false,
        success: function (res) {
            if (res.success) {
                showToast('success', res.message || 'CAPA successfully updated!');
                ...
                $submitBtn.prop('disabled', false).text('Update'); // aktifkan lagi
            }
        },
        error: function (err) {
...
$('#department').on('change', function () {
    ...
    $.ajax({
        ...
        success: function (users) {
            ...
            // 🔥 isi dropdown (pakai new Option biar select2 kenal)
            $.each(users, function (i, user) {
                $rep1.append(new Option(user.name, user.id, false, false));
                $rep2.append(new Option(user.name, user.id, false, false));
            });

            // =========================
            // 🔥 SET SELECTED REP 1 (FIX)
            // =========================
            if (selectedRep1) {
                let user1 = users.find(u => String(u.id) === String(selectedRep1));
                if (user1) {
                    $rep1.val(String(user1.id)).trigger('change');
                }
            }

            // =========================
            // 🔥 SET SELECTED REP 2 (FIX)
            // =========================
            if (selectedRep2) {
                let user2 = users.find(u => String(u.id) === String(selectedRep2));
                if (user2) {
                    $rep2.val(String(user2.id)).trigger('change');
                }
            }
