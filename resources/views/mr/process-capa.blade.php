...
<div class="flex sm:items-center">
    ...
    <span class="inline-flex items-center justify-center gap-1.5
        ...
        <i class="fa-regular {{ $icon }} text-xs {{ $iconColor }}"></i>
...
<!-- SOURCE & CATEGORY -->
<div class="col-span-2 space-y-8 mt-6 mb-8">
...
<!-- DETAIL, PROBLEM, RCA -->
<div class="col-span-2 space-y-6 mb-4">
 ...
 <form id="capa-form" enctype="multipart/form-data">
      ...
      @method('PUT')
         ...
         <div class="col-span-2">
          <label class="block text-sm font-medium text-gray-500 mb-1">Root Cause Analysis (RCA) <small class="text-red-600">*</small></label>
         <textarea name="rca" id="rca" rows="5" required
class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm px-4 py-2 resize-none"
>{{ old('rca', $capa->rca?->description) }}</textarea>
  ...
  <div class="row-span-2">
     <label class="block text-sm font-medium text-gray-500 mb-1">Corrective Action (CA)<small class="text-red-600">*</small></label>
         <textarea name="corrective_action" id="corrective_action" rows="5" required
class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm px-4 py-2 resize-none"
>{{ old('corrective_action', $capa->ca?->description) }}</textarea>


  </div>

   <div>
          <label class="block text-sm font-medium text-gray-500 mb-1">Person-in-Charge (PIC)</label>
          <select id="ca_pic" name="ca_pic" required
            class="w-full px-3 py-2 border border-gray-300 text-sm rounded shadow-sm"
            >
                <option value="">-- Select PIC --</option>
                                @foreach($users as $user)
                                   <option value="{{ $user->id }}"
 {{ old('ca_pic', $capa->ca?->pic) == $user->id ? 'selected' : '' }}>
 {{ $user->name }}
</option>
                                @endforeach
          </select>
   ...
   <div>
          <label class="block text-sm font-medium text-gray-500 mb-1">
            Due Date <small class="text-red-600">*</small>
          </label>
          <input type="date" name="ca_due_date" id="ca_due_date" required
                 class="w-full px-3 py-2 border border-gray-300 rounded shadow-sm focus:ring-indigo-500 focus:border-indigo-500" 
                value="{{ old('ca_due_date', $capa->ca?->due_date) }}"/>
 ...
 <div class="col-span-2 grid grid-cols-2 gap-2">
  <div class="row-span-2">
     <label class="block text-sm font-medium text-gray-500 mb-1">Preventive Action (PA)<small class="text-red-600">*</small></label>
          <textarea name="preventive_action" rows="5" required
...
>{{ old('preventive_action', $capa->pa?->description) }}</textarea>

  </div>

   <div>
          <label class="block text-sm font-medium text-gray-500 mb-1">Person-in-Charge (PIC)</label>
          <select id="pa_pic" name="pa_pic" required
            class="w-full px-3 py-2 border border-gray-300 text-sm rounded shadow-sm"
            >
                <option value="">-- Select PIC --</option>
                                @foreach($users as $user)
                                    <option value="{{ $user->id }}"
 {{ old('pa_pic', $capa->pa?->pic) == $user->id ? 'selected' : '' }}>
 {{ $user->name }}
</option>
                                @endforeach
          </select>
   ...
   <div>
          ...
          <input type="date" name="pa_due_date" id="pa_due_date" required
                 class="w-full px-3 py-2 border border-gray-300 rounded shadow-sm focus:ring-indigo-500 focus:border-indigo-500"
                 min="{{ date('Y-m-d') }}"
                 value="{{ old('pa_due_date', $capa->pa?->due_date) }}"/>
        </div>
...
$('#capa-form').off('submit').on('submit', function (e) {
    ...
    $.ajax({
        url: '{{ route("mr.capa.process.save", $capa->id) }}',
        ...
        success: function (res) {
            if (res.success) {
                showToast('success', res.message || 'CAPA successfully Process!');
                setTimeout(() => {
                    window.location.href = '{{ route("mr.capa.index") }}';
                }, 2000);
            } else {
                showToast('error', res.message || 'Gagal process CAPA.');
                  $btn.prop('disabled', false).removeClass('opacity-50 cursor-not-allowed').html(originalText);
            }
        },
        error: function (err) {
            console.error(err.responseText);
            const msg = err.responseJSON?.message || 'Terjadi kesalahan saat process.';
            showToast('error', msg);
            $btn.prop('disabled', false).removeClass('opacity-50 cursor-not-allowed').html(originalText);
        }
    });
